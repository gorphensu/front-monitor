...
import MPageEngineRender from '~/src/model/summary/page_engine_render'
import MPageEngineRenderSummary from '~/src/model/summary/page_engine_render_summary'
import MProject from '~/src/model/project/project'
import DATE_FORMAT from '~/src/constants/date_format'
import _ from 'lodash'
import summary from '~/src/routes/api/project/summary'

export default class PageEngineRenderSummary extends Base {
  ...
  static get description() {
    return '[按天] 基于数据库统计引擎表单页面加载渲染情况'
  }

  async execute(args, options) {
    Logger.info('开始执行 Summary:PageEngineRenderSummary')
    ...
    for (let rawProject of rowProjectList) {
      ...
      let res = await MPageEngineRender.getList(projectId, startAt, endAt)
      if (res.length) {
  ...
  async addOrReplaceRecord(projecetId, data) {
    return await MPageEngineRenderSummary.updateRecord(projecetId, data)
  }
    ...
    for (let record of records) {
      try {
        ...
        if (isSuccess) {
          ...
          Logger.info('summary page_engine_rener_summary 插入失败')
        }
        processRecordCount++
      } catch (e) {
        Logger.info('summary page_engine_rener_summary 插入失败', e)
        processRecordCount++
      }
    }
